	...
	class CyberChimpsTwitterBar {
		...
		public function render_display() {
			...
			elseif( is_wp_error( $latest_tweet ) ) {
				...
				?>
				<div id="twitter_container" class="row">
					<div id="twitter_bar" class="col-md-12">
						<div id="twitter_text">
							<div class="ul">
								<?php
								if( $latest_tweet ) {
									...
									foreach( $latest_tweet as $tweet ) {
										...
										if( $tweet_text ) {
											echo '<div class="' . $count_class . ' row">';
											echo '<div class="twitter-img col-lg-1 col-md-1"><a href="' . esc_url( $gravtar_url ) . '">' .
												'<img alt="' . $gravtar_name . '" title="' . $gravtar_name . '" src="' . esc_url( $image_url ) . '" /></a>';
											if( $follower_toggle ) {
												echo '<p class="twitter-followers">' . sprintf( __( '%1$s followers', 'cyberchimps_elements' ), $follower_count ) . '</p>';
											}
                                                                                        echo '</div>';
											echo '<div class="tweet col-lg-11 col-md-11"><a href="' . esc_url( $user_permalink ) . '"> ';
											echo $screen_name . '</a> - ' . wp_kses( $tweet_text, array( 'a' => array( 'href' => array() ) ) ) . ' <p><small><a href="' . $tweet_permalink . '">' . sprintf( __( '%s ago', 'cyberchimps_elements' ), human_time_diff( strtotime( $tweet['created_at'] ), current_time( 'timestamp' ) ) ) . '</a></small></p></div>';
											echo '</div>';
										}
										$count++;
									}
		...
		public function meta_boxes() {
			...
			if( class_exists( 'Cyberchimps_Meta_Box' ) ) {
				...
				$config = array(
					...
					'fields'         => apply_filters( 'cyberchimps_twitter_bar_metabox_fields', $fields, 'twitter_bar' ), // list of meta fields (can be added by field arrays)
