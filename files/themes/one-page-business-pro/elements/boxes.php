...
function cyberchimps_init_boxes_post_type() {
	...
	$box_fields = array(
		...
		array(
			'type' => 'text',
			'id' => 'cyberchimps_box_readmore_text',
			'class' => '',
			'name' => __( 'Box Read More Text', 'cyberchimps_elements' )
		),
		array(
			'type'  => 'checkbox',
	...
	$box_config = array(
		...
		'fields'         => apply_filters( 'cyberchimps_boxes_single_metabox_fields', $box_fields), // list of meta fields (can be added by field arrays)
	...
	$page_config = array(
		...
		'context'        => 'normal', // where the meta box appear: normal (default), advanced, side; optional
		'priority'       => 'high', // order of meta box: high (default), low; optional
		'fields'         => apply_filters( 'cyberchimps_boxes_metabox_fields', $page_fields, 'boxes' ), // list of meta fields (can be added by field arrays)
		'local_images'   => false, // Use local or hosted images (meta box images for add/remove)
		'use_with_theme' => true //change path if used with theme set to true, false for a plugin or anything else for a custom path(default false).
	);
...
function cyberchimps_boxes_render_display() {
	...
	<div id="boxes_container">
		<?php do_action( 'boxes_begin_container' ); ?>
		<div class="boxes row">
			<?php
			if( $boxes && $customcategory != '' ):
				foreach( $boxes as $box ):
					...
					$box_read_more_text = get_post_meta( $box->ID, 'cyberchimps_box_readmore_text', true );
					?>

					<div id="box<?php echo $box_id_counter ?>" class="box <?php echo $boxes_span; ?> col-xs-12">
						...
						<a href="<?php echo $box_url; ?>">
							...
							<a href="<?php echo $box_url; ?>" class="box-link">
								<?php endif; ?>
								<?php if ( $box_title != '' && $box_title ): ?>
									<h2 class="box-widget-title"><?php echo $box_title; ?></h2>
								<?php endif; ?>								
								<?php if ($box_url != '' && !$box_total_link): ?>
							</a>
						<?php endif; ?>
							<?php if ( $box_text != '' && $box_text ): ?>
					...
					<?php endif; ?>
						<?php if ( $box_url != '' && ! $box_total_link && ! empty( $box_read_more_text ) ) { ?>
								<a href="<?php echo $box_url; ?>" title="<?php echo apply_filters( 'box_redmore_text', $box_read_more_text ); ?>" >
									<?php echo apply_filters( 'cyberchimps_boxes_readmore', $box_read_more_text ); ?>
								</a>
						<?php } ?>
				...
				<div class="box <?php echo $boxes_span;?> default-box">
		...
		<!-- end boxes -->
		<?php do_action( 'boxes_end_container' ); ?>
	</div><!-- end row -->
<?php
}
