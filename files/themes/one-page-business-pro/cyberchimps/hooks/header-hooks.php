	...
	foreach ( $social as $key => $value ) {
		...
		if ( !empty( $value['set'] ) ) {
			...
			// Create the output
                        if($design == 'hexagon')
                        {
                            $output .= '<span class="hex hex-xs hex-'.$key.'"><a href="' . esc_url( $social[$key]['url'] ) . '"' . ( "email" != $key ? ' target="_blank"' : '' )
				. ' title="' . esc_attr( $link_title . ' ' . ucwords( $title ) ) . '" class="latest_symbol ' . $key . '"></a></span>';
                        }
                        else if($design == 'pentagon')
                        {
                            $output .= '<span class="pentagon pentagon-xs pentagon-'.$key.'"><a href="' . esc_url( $social[$key]['url'] ) . '"' . ( "email" != $key ? ' target="_blank"' : '' )
				. ' title="' . esc_attr( $link_title . ' ' . ucwords( $title ) ) . '" class="latest_symbol ' . $key . '"></a></span>';
                        }
                        else{
