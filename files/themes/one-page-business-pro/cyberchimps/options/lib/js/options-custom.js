...
// sets up scrolling left menu
	...
	// Script to hide show the Google Menu Font input depending on value of the Menu select
	var font = $('#font_family_menu_face').val();
	if (font != 'Google Fonts') {
		$('#google_font_menu').parent().hide();
	}
	else {
		$('#google_font_menu').parent().show();
	}
	$('#font_family_menu_face').change(function () {
		var font_change = $(this).val();
		if (font_change != 'Google Fonts') {
			$('#google_font_menu').parent().hide();
		}
