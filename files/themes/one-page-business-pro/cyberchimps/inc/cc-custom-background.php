...
class CC_Custom_Background {
	...
	protected function get_radio_fields() {
		...
		$radios = array( 'none', 'noise', 'blue', 'dark', 'space', 'debut_light', 'silk', 'grid', 'pet' );
...
function pet_background_image( $options ) {
	...
	$options = array(
			...
			'pet' => $imagepath . 'backgrounds/thumbs/pet.jpg',
			);
	return $options;
}
add_filter( 'cyberchimps_background_image', 'pet_background_image' );

// default background color
function pet_default_color() {
	$color = 'f7f7f7';
	return $color;
}
add_filter( 'default_background_color', 'pet_default_color' );
