...
function cyberchimps_add_thumbnail_size() {
	add_image_size( 'cyberchimps-featured-thumb', 100, 80, true );
	add_image_size( 'cyberchimps-headline-thumb', 200, 225, true );
}
...
function cyberchimps_options_help_text() {
	...
	$text .= '<div class="cc_help_section">
						<div class="row"><div class="col-md-6">
							...
							<div class="cc_help_caption pull-right"><p>' . __( 'Instructions', 'cyberchimps_core' ) . '</p></div>
						</div>
						<div class="col-md-6">
							...
							<div class="cc_help_caption pull-right"><p>' . __( 'Support', 'cyberchimps_core' ) . '</p></div>
						</div>
						</div>';
...
//Set showcase background
function cyberchimps_showcase_background(){
	$showcase_custombackground = cyberchimps_get_option('showcase_background');
?>
	<style type="text/css" media="all">
		#showcase_section{ 
			background : url("<?php echo $showcase_custombackground; ?>") no-repeat scroll 0 0 / cover;
		}	
	</style>
<?php
}
add_action('wp_head','cyberchimps_showcase_background');

//function to alter query for magazine element's pagination
add_action( 'pre_get_posts', 'set_magazine_post_data');

function set_magazine_post_data( $query ) 
{ 
	if ( is_admin() || !$query->is_main_query() )
        return;

	
	$frontpage_set = get_option('show_on_front');

	// if page is static front page
	if($frontpage_set == 'page')
	{
		$frontpage_id = get_option( 'page_on_front' );
		if($frontpage_id != 0)
		{
			$page_section_order = get_post_meta( $frontpage_id, 'cyberchimps_page_section_order', true );
		}
	}
	
	// for blog page -from theme options
	if($frontpage_set == 'posts')
	{
		$blog_section_order = cyberchimps_get_option( 'blog_section_order' );
	}	
	
	if( ($frontpage_set == 'posts') || ($frontpage_set == 'page' &&  $frontpage_id == 0) )
	{
		if ( is_home() && in_array( 'magazine', $blog_section_order ))
    	{

			$column_no = cyberchimps_get_option('blog_magazine_no_of_columns');
			$row_no = cyberchimps_get_option('blog_magazine_no_of_rows');
			$wide_post_toggle = cyberchimps_get_option('blog_magazine_wide_post');
			$wide_post_no     = cyberchimps_get_option('blog_magazine_no_of_wide_posts');
			$post_no_total = ( $row_no * $column_no ) + ( $wide_post_toggle ? $wide_post_no : 0 );

		    $query->set( 'posts_per_page', $post_no_total );
			return;
    	}
	}	
	
	if( $frontpage_set == 'page' && $frontpage_id != 0 )
	{
		if ( in_array( 'magazine', $page_section_order ))
    	{
			$column_no = get_post_meta( $frontpage_id, 'cyberchimps_magazine_no_of_columns', true );
			$row_no = get_post_meta( $frontpage_id, 'cyberchimps_magazine_no_of_rows', true );
			$wide_post_toggle = get_post_meta( $frontpage_id, 'cyberchimps_magazine_wide_post_toggle', true );
			$wide_post_no     = get_post_meta( $frontpage_id, 'cyberchimps_magazine_no_of_wide_posts', true );
			$post_no_total = ( $row_no * $column_no ) + ( $wide_post_toggle ? $wide_post_no : 0 );
